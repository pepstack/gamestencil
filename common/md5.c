...
void MD5HashString(unsigned char *source, unsigned int size, unsigned int seed, char hash[MD5_HASH_SIZE])
{
    int  i;
    char tmp[3];
    MD5_CTX  m_md5;
    ...
    *hash = 0;
    ...
    MD5Init(&m_md5, seed);
    MD5Update(&m_md5, source, size);
    MD5Final(&m_md5);

    for ( i = 0 ; i < 16 ; i++ ) {
        sprintf(tmp, "%02x", m_md5.digest[i]);
        *psz++ = tmp[0];
        *psz++ = tmp[1];
    }
    hash[MD5_HASH_SIZE-1] = 0;
}


static const char* internalMD5HashFile (FILE *f, unsigned int seed , char hash[MD5_HASH_SIZE])
{
    int i;
    char tmp[3];
    MD5_CTX  ctx;
    char buf[MD5_BUFFSIZE] ;
    unsigned int trb = 0 ;
    ...
    *hash = 0;
    ...
    for ( ; ; ) {
        int rb = (int) fread ( buf , 1 , MD5_BUFFSIZE , f ) ;
        ...
        MD5Update ( &ctx , (unsigned char *) buf , rb ) ;

        if ( rb < MD5_BUFFSIZE || ( size > 0 && trb >= size ) ) {
            break ;
        }
    }
...
const char* MD5HashFile (const char *fn , unsigned int seed, char hash[MD5_HASH_SIZE])
{
    FILE * fp = fopen( fn , "rb" ) ;
    if ( fp ) {
