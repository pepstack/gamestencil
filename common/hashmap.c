 ...
 */
static uint32_t crc32(const unsigned char *s, unsigned int len)
{
    unsigned int i;
    uint32_t crc32val = 0;
    for (i = 0; i < len; i ++) {
 ...
 *   with Knuth's Multiplicative Method
 */
#define Jenkins_Mix_Key(key) do { \
        key += (key << 12); \
        key ^= (key >> 22); \
        key += (key << 4); \
        key ^= (key >> 9); \
...
/**
 * Hashing function for integer
 */
int get_int_hash(void * l, int hashsize)
{
    if (sizeof(l) == sizeof(uint64_t)) {
        uint32_t hikey = (uint32_t) ((((uint64_t)(l)) >> 32) & 0xFFFFFFFF);        
        uint32_t lokey = (uint32_t)(((uint64_t)(l)) & 0xFFFFFFFF);

        Jenkins_Mix_Key(hikey);
        Jenkins_Mix_Key(lokey);
...
static int _hashmap_hash(hmap_t in, char* key)
{
    ...
    /* Find the best index */
    curr = get_string_hash(key, strlen(key), m->table_size);
...
/**
 * Case a pointer to key
 * Return char * as key
 */
char* ptr_addr_to_key(void_ptr ptr, char* key)
{
#ifdef _MSC_VER
    #pragma warning(push)
...
/**
 * Remove all elems in the hashmap
 */
void hashmap_clear(hmap_t in, hmap_callback_func fnFreeValue, void_ptr arg)
{
    hashmap_elem_t *elem;
    void_ptr data;
    hashmap_map_t* m = (hashmap_map_t*) in;
...
{
    hashmap_clear(in, fnFreeValue, arg);
    free(((hashmap_map_t *) in)->elems);
    free(in);
}
